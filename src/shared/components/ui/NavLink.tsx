...
export default function NavLink({ children, href, icon }: Props) {
    ...
    return (
        <Link
            href={href}
            className={`group relative flex items-center gap-2 text-md uppercase tracking-widest transition-all duration-300 ${active
                    ? "text-amber-400"
            ...
            {icon && (
                <span
                    className={`
                        ...
                        ${active ? "text-amber-400" : "text-zinc-500 group-hover:text-amber-300"}
                    `}
                >
                    {icon}
                </span>
